...
export async function findUserByEmail(email) {
  ...
  if (await ensureSchema()) { try { const sql = await getSql(); const { rows } = await sql`SELECT email, password_hash AS "passwordHash", business_id AS "businessId" FROM hamdan_users WHERE email = ${normalized} LIMIT 1`; return rows[0] || null; } catch (error) { console.error('User lookup failed; using memory fallback:', error?.message || 'Unknown error'); } }
  return users.get(normalized) || null;
}
export async function findUserByBusinessId(businessId) {
  if (await ensureSchema()) { try { const sql = await getSql(); const { rows } = await sql`SELECT email, business_id AS "businessId" FROM hamdan_users WHERE business_id = ${businessId} LIMIT 1`; return rows[0] || null; } catch (error) { console.error('User business lookup failed; using memory fallback:', error?.message || 'Unknown error'); } }
  for (const user of users.values()) if (user.businessId === businessId) return { email: user.email, businessId: user.businessId };
  return null;
}
export async function saveUser(user) {
  ...
  if (await ensureSchema()) { try { const sql = await getSql(); await sql`INSERT INTO hamdan_users (email, password_hash, business_id) VALUES (${normalized}, ${user.passwordHash}, ${user.businessId})`; return record; } catch (error) { if (String(error?.message || '').toLowerCase().includes('duplicate') || String(error?.code || '') === '23505') throw new Error('An account with that email already exists.'); console.error('User save failed; using memory fallback:', error?.message || 'Unknown error'); } }
  if (users.has(normalized)) throw new Error('An account with that email already exists.');
  users.set(normalized, record); return record;
}
