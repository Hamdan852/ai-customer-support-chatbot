...
import { findUserByEmail, findUserByBusinessId, getBusinessConfig, saveBusinessConfig, saveUser } from './business-store.js';
...
export default async function handler(req, res) {
  ...
  try {
    if (action === 'signup') {
      ...
      setSessionCookie(res, createSession(businessId));
      return res.status(201).json({ success: true, businessId, email });
    }
    ...
    if (action === 'login') {
      ...
      setSessionCookie(res, createSession(user.businessId));
      return res.status(200).json({ success: true, businessId: user.businessId, email: user.email });
    }
    ...
    if (action === 'me') {
      ...
      const user = await findUserByBusinessId(businessId);
      const config = await getBusinessConfig(businessId);
      return res.status(200).json({ authenticated: true, businessId, email: user?.email || config?.contactEmail || '', business: config });
    }

    return res.status(400).json({ error: 'Unknown authentication action.' });
  } catch (error) {
    console.error('Session API error:', error?.message || 'Unknown error');
    return res.status(500).json({ error: error?.message || 'Authentication request failed.' });
  }
}
