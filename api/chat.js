...
function localSupportAnswer(message, mode, config) {
  ...
  const business = config?.businessName || 'the business';
  if (mode === 'real-estate') {
    ...
    return `I can help with ${business}'s property-search preferences, showing requests, buyer or seller lead qualification, and agent handoff. Tell me what you need and I’ll help organize the request.`;
  }
  if (/hello|hi|hey|good morning|good afternoon|good evening/.test(text)) return `Hello! 👋 I’m the ${business} AI Support Assistant. How can I help you today?`;
  if (/shipping|delivery|deliver/.test(text)) return 'For shipping details, please provide your order number or tell me what you need to know. I can explain the available options once the company’s current policy is provided.';
  ...
  if (/hour|open|close|support/.test(text)) return 'I’m ready to help. For current opening hours or direct support contact, I’ll use the business information configured by the company.';
